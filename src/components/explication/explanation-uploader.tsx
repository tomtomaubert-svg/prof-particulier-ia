...
export function ExplanationUploader() {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const cameraInputRef = useRef<HTMLInputElement>(null);
  const [preview, setPreview] = useState<string | null>(null);
  const [depth, setDepth] = useState<"simple" | "normal" | "approfondi">("normal");
      ...
      {preview ? (
        <Card>
          <CardBody className="space-y-4">
            ...
            <img src={preview} alt="Notion à expliquer" className="w-full rounded-[var(--radius-md)] max-h-96 object-contain bg-surface-raised" />
            <div>
              <p className="text-sm font-medium mb-2">Niveau d&apos;explication</p>
                ...
                {DEPTHS.map((d) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      depth === d.value ? "border-primary bg-primary-soft" : "border-border"
                    )}
                  >
                    {d.label}
                  </button>
